    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return GestureDetector(
          ...
          onTapDown:
              (details) => _update(details.localPosition, Size(side, side)),
          onPanStart:
              (details) => _update(details.localPosition, Size(side, side)),
          onPanUpdate:
              (details) => _update(details.localPosition, Size(side, side)),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final basePaint =
        Paint()
          ..shader = const RadialGradient(
            colors: [Color(0x331DE9B6), Color(0x120C1522), Color(0x08000000)],
            stops: [0, 0.7, 1],
          ).createShader(
    ...
    final ringPaint =
        Paint()
          ...
          ..color = const Color(
            0x6636E6C5,
          ).withValues(alpha: 0.18 + (0.42 * alpha));

    final linePaint =
        Paint()
          ..style = PaintingStyle.stroke
          ..strokeWidth = 1.1
          ..color = Colors.white.withValues(alpha: 0.07 + (0.18 * alpha));
    ...
    final glowPaint =
        Paint()
          ..color = const Color(
            0xFF1DE9B6,
          ).withValues(alpha: dragging ? 0.30 : 0.16)
    ...
    final knobPaint =
        Paint()
          ..shader = RadialGradient(
            colors: [
              const Color(0xFF3DFFB6).withValues(alpha: 0.92),
              const Color(0xFF15B7FF).withValues(alpha: 0.82),
            ],
          ).createShader(
